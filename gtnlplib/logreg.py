...
def compute_py(x,weights,labels):
    ...
    """
    _, scores = predict(x, weights, labels)
    probabilities = {}
    for label in labels:
        probabilities[label] = np.exp(scores[label]) / np.exp(logsumexp(scores.values()))
...
def estimate_logreg(x,y,N_its,learning_rate=1e-4,regularizer=1e-2,lazy_reg=True):
    ...
    for it in xrange(N_its):
        for i,(x_i,y_i) in enumerate(zip(x,y)):
            ...
            y_feature_vector = make_feature_vector(x_i, y_i)
            p_y = compute_py(x_i, weights, all_labels)
            updates = defaultdict(float)
            for label in all_labels:
                probability = p_y[label]
                label_feature_vector = make_feature_vector(x_i, label)
                for key in label_feature_vector:
                    updates[key] = (y_feature_vector[key] if key in y_feature_vector else 0) - probability * label_feature_vector[key]
            for key in updates:
                weights[key] += learning_rate * updates[key]
